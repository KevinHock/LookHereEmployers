...
char find_xor_key(char *);
char find_xor_key_with_print(char *, char*);

//Assume most frequent letters will appear more than less frequent letters
char find_xor_key(char *buff){
	int value;
	int byte[8];
	int key_byte[8];
	int byte_index;
	...
	return best_char_key;
}

//Assume most frequent letters will appear more than less frequent letters
char find_xor_key_with_print(char *buff, char *answer){
	int value;
	int byte[8];
	int key_byte[8];
	int byte_index;
	...
	for (byte_index = 0; byte_index < strlen(buff); byte_index+=2){
		...
		//printf("%c", (char)value);
		char *blah = calloc(sizeof(char)*5, 1);
		blah[0] = (char)value;
		blah[1] = '\0';
		strcat(answer, blah);
	}
...
void fill_byte_by_nibs(int *byte, char char_a, char char_b){
	...
	if(       (char_value_a < 58)  && (47 < char_value_a) ){
		char_value_a -=48;
	}else if( (char_value_b < 71) && (64 < char_value_b) ){
		char_value_b -= 55;
	}else if( (char_value_a < 103) && (96 < char_value_a) ){
		char_value_a -= 87;
	}else{
		printf("Invalid input at %d because char_value_b is %x\n", __LINE__, char_value_b);
		exit(-1);
	}
